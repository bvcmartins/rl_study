# LOGGING AND VISUALIZATION UTILITIES FOR DEFENDER PPO CNN
# Adapted from space_invaders_ppo_cnn logging utilities
...
def save_checkpoint(artifacts_dir, episode, model, optimizer, episode_rewards, run_id, total_updates):
    ...
    checkpoint_path = artifacts_path / f'defender_ppo_cnn_checkpoint_ep{episode}.pth'
...
def log_final_results(model, episode, mean_reward, episode_rewards, artifacts_dir, run_id):
    ...
    final_model_path = artifacts_path / 'final_defender_ppo_cnn_model.pth'
    torch.save({
        'model_state_dict': model.state_dict(),
        'run_id': run_id,
