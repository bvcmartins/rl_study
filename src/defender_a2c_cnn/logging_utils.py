# COMPREHENSIVE LOGGING AND VISUALIZATION UTILITIES FOR PONG A2C CNN
# This module contains all logging, visualization, and tracking functions for training monitoring
# Simplified for basic A2C implementation (no GAE)
...
def log_model_info(model):
    ...
    # Calculate parameter breakdown by component (A2C-specific)
...
def log_a2c_losses(policy_loss, value_loss, entropy_loss, total_loss, episode, total_updates):
    """Log A2C loss components to wandb."""
    wandb.log({
...
def save_checkpoint(artifacts_dir, episode, model, optimizer, episode_rewards, run_id, total_updates):
    ...
    checkpoint_path = artifacts_path / f'pong_a2c_cnn_basic_checkpoint_ep{episode}.pth'
...
def log_final_results(model, episode, mean_reward, episode_rewards, artifacts_dir, run_id):
    ...
    final_model_path = artifacts_path / 'final_pong_a2c_cnn_basic_model.pth'
    torch.save({
        'model_state_dict': model.state_dict(),
        'run_id': run_id,
